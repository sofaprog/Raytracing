...
void initializeDefaultScene (out STriangle triangles[12], out SSphere spheres[21])
{
...
float sphereRadius = 0.5;
    int yellowMatIdx = 8;
    float verticalShift = 0.5; // Сдвиг вверх всей буквы
    
    // Левый столбец (x = -3) - 5 сфер
    spheres[2].Center = vec3(-3.0, 3.0 + verticalShift, -7.9);
    spheres[3].Center = vec3(-3.0, 2.0 + verticalShift, -7.9);
    spheres[4].Center = vec3(-3.0, 1.0 + verticalShift, -7.9);
    spheres[5].Center = vec3(-3.0, 0.0 + verticalShift, -7.9);
    spheres[6].Center = vec3(-3.0, -1.0 + verticalShift, -7.9);

    // Средний столбец (x = 0) - 5 сфер
    spheres[7].Center = vec3(0.0, 3.0 + verticalShift, -7.9);
    spheres[8].Center = vec3(0.0, 2.0 + verticalShift, -7.9);
    spheres[9].Center = vec3(0.0, 1.0 + verticalShift, -7.9);
    spheres[10].Center = vec3(0.0, 0.0 + verticalShift, -7.9);
    spheres[11].Center = vec3(0.0, -1.0 + verticalShift, -7.9);

    // Правый столбец (x = 3) - 5 сфер
    spheres[12].Center = vec3(3.0, 3.0 + verticalShift, -7.9);
    spheres[13].Center = vec3(3.0, 2.0 + verticalShift, -7.9);
    spheres[14].Center = vec3(3.0, 1.0 + verticalShift, -7.9);
    spheres[15].Center = vec3(3.0, 0.0 + verticalShift, -7.9);
    spheres[16].Center = vec3(3.0, -1.0 + verticalShift, -7.9);

    // Нижние перемычки (3 сферы)
    spheres[17].Center = vec3(-1.0, -1.0 + verticalShift, -7.9);
    spheres[18].Center = vec3(1.0, -1.0 + verticalShift, -7.9);
    spheres[19].Center = vec3(2.0, -1.0 + verticalShift, -7.9);
    spheres[20].Center = vec3(-2.0, -1.0 + verticalShift, -7.9);

    // Установка параметров для всех сфер буквы
    for(int i = 2; i < 21; i++) {
        spheres[i].Radius = sphereRadius;
        spheres[i].MaterialIdx = yellowMatIdx;
    }
...
void initializeDefaultLightMaterials(out SLight light, out SMaterial materials[9]) 
{
	...
	materials[2].Color = vec3(1.0, 0.0, 1.0);  
	...
	materials[8].Color = vec3(1.0, 1.0, 0.0); // Ярко-жёлтый
...
{ 
    ...
    for(int i = 0; i < 21; i++) {   
        ...
        if(IntersectSphere(sphere, ray, start, final, test) && test < intersect.Time) {       
            ...
            SMaterial mat = Materials[sphere.MaterialIdx]; // Исправлено
            intersect.Color = mat.Color;        
            intersect.LightCoeffs = mat.LightCoeffs;
            intersect.ReflectionCoef = mat.ReflectionCoef;   
            intersect.RefractionCoef = mat.RefractionCoef;       
